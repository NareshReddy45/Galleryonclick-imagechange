...
function Gallery() {
  ...
  const [selectedAltText, setSelectedAltText] = useState(
    imagesList[0].imageAltText,
  )

  const handleThumbnailClick = (imageUrl, imageAltText) => {
  ...
  return (
    <div className="bg-container">
      <img src={selectedImage} alt={selectedAltText} className="main-image" />
      ...
      <ul className="thumbnails-container">
        {imagesList.map(item => (
          <ThumbnailItem
            ...
            onClick={() =>
              handleThumbnailClick(item.imageUrl, item.imageAltText)
            }
          />
        ))}
      </ul>
